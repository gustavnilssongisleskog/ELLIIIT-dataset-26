...
for mn in tqdm(range(0, n_test_set_positions)):
    new_mn = True
        ...
        if anchor_selection_tech== 'All':
            ...
            if single_mn_info_matrix.shape[0] == n_anchor_nodes:
                ...
                if info_matrix_3D_first_start:
                    ...
                    value_matrix = np.vstack((value_matrix, value_list))
                    anchor_selection_matrix = np.vstack((anchor_selection_matrix, anchor_selection_list))
        elif anchor_selection_tech == 'STMR':
            # Get list with peak idxs of sig peaks
            peaks_idx_list = np.load(
                path_PP_data + 'peak_idxs_mic' + str(rx) + 'speaker' + str(sp) + 'notAGCsimulation' + str(
                    sim_nr) + '.npy')
            ...
            # Get peak heights
            peak_vals = corr_graph[peaks_idx_list]

            # Sort biggest first and take first 2
            sorted_peak_vals = -np.sort(-peak_vals)

            # Devide to Get STMR value
            STMR_val = -sorted_peak_vals[1]/sorted_peak_vals[0]

            if new_mn:
                single_mn_info_matrix = np.array([STMR_val])
                new_mn = False
            else:
                single_mn_info_matrix = np.vstack((single_mn_info_matrix, STMR_val))
...
np.save(extra_save_path+'info_matrix.npy', info_matrix)
np.save(extra_save_path+'value_matrix.npy', value_matrix)
np.save(extra_save_path+'anchor_selection_matrix.npy', anchor_selection_matrix)