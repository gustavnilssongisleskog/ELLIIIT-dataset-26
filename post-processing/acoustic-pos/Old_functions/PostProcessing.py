...
path_corr_not_norm = save_loc+'not_normalized_correlations\\'
path_LPF_curve = save_loc+'LPF_curve\\'
path_dist_th = save_loc+'dist_th\\'
path_LPF_curve_per_simulation = save_loc+'LPF_curves_per_simulation\\'
path_LPF_curve_per_simulation_downsampled = save_loc+'LPF_curves_per_simulation_downsampled\\'
...
for sim_nr in tqdm(range(0, n_speakers)):
    ...
    for rx in range(n_mics):
        for sp in range(n_speakers_one_sim):
            audio_wakeup = np.array(rx_audio_wake[rx, :])
            np.save(path_wakeuppart + 'rx_wakeup_mic' + str(rx) + 'speaker' + str(sp) + 'simulation' + str(sim_nr) + '.npy',
                    audio_wakeup)
        ...
        for sp in range(n_speakers_one_sim):
            ...
            if config['AGC'] and not config['xor']:
                ...
                if config['absolute_corr']:
                    ...
                    if config['get_nonnormalized_compr']:
                        corr_val_not_norm = lf.pulse_compres_not_norm(chirp_orig_resampl, rx_chirp_agc)
                else:
                    corr_val = lf.pulse_compres_not_abs(chirp_orig_resampl, rx_chirp_agc)
                    if config['get_nonnormalized_compr']:
                        corr_val_not_norm = lf.pulse_compres_not_abs_not_norm(chirp_orig_resampl, rx_chirp_agc)
                ...
                if config['save_corr_val']:
                    ...
                    if config['get_nonnormalized_compr']:
                        np.save(path_corr_not_norm + 'corr_val_AGC_mic' + str(rx) + 'speaker' + str(sp) + 'simulation' + str(
                            sim_nr) + '.npy', corr_val_not_norm)
                ...
                if config['absolute_corr']:
                    ...
                    if config['get_nonnormalized_compr']:
                        corr_val_not_norm = lf.pulse_compres_not_norm(chirp_orig_resampl, audio_row)
                else:
                    corr_val = lf.pulse_compres_not_abs(chirp_orig_resampl, audio_row)
                    if config['get_nonnormalized_compr']:
                        corr_val_not_norm = lf.pulse_compres_not_abs_not_norm(chirp_orig_resampl, audio_row)

                if config['save_corr_val']:
                    # Save values
                    np.save(path_corr + 'corr_val_mic' + str(rx) + 'speaker' + str(sp) + 'simulation'+str(sim_nr)+'.npy', corr_val)
                    if config['get_nonnormalized_compr']:
                        np.save(path_corr_not_norm + 'corr_val_mic' + str(rx) + 'speaker' + str(sp) + 'simulation' + str(
                                sim_nr) + '.npy', corr_val_not_norm)
            ...
            corr_filtered = lf.LPF(corr_val, 'lowpass', config['order_lpf'], config['cutoff_lpf'], fs_mic)
            ...
            if config['plot_pulse_compr'] or plot_first :
                ...
                lf.plot_corr_LPF_svg("Correlation at the " + str(rx) + "th mic from the " + str(sp) + "th speaker, simulation "+str(sim_nr), corr_val, corr_filtered, "Correlation_mic_{}from_source_{}sim_{}".format(rx, sp, sim_nr))
            ...
            if config['plot_nonnormalized_compr']:
                lf.plot_corr_easy("Correlation at the " + str(rx) + "th mic from the " + str(sp) + "th speaker, simulation " + str(sim_nr), corr_val_not_norm)
